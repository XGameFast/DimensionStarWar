...
public class BMFont2
{
	...
	[HideInInspector][SerializeField] List<BMGlyph2> mSaved = new List<BMGlyph2>();

	// Actual glyphs that we'll be working with are stored in a dictionary, making the lookup faster
	Dictionary<int, BMGlyph2> mDict = new Dictionary<int, BMGlyph2>();
	...
	public List<BMGlyph2> glyphs { get { return mSaved; } }

	/// <summary>
	/// Helper function that retrieves the specified glyph, creating it if necessary.
	/// </summary>

	public BMGlyph2 GetGlyph (int index, bool createIfMissing)
	{
	...
	public BMGlyph2 GetGlyph (int index) { return GetGlyph(index, false); }
