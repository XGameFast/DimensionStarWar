...
////// Lighting:
////// Emissive:
                float node_4041 = pow(1.0-max(0,dot(normalDirection, viewDirection)),0.7);
                float4 node_7889 = _Time;
                float2 node_7347 = (i.uv0+node_7889.g*float2(0,-1));
                float4 _node_8894_var = tex2D(_node_8894,TRANSFORM_TEX(node_7347, _node_8894));
                float3 emissive = ((pow(1.0-max(0,dot(normalDirection, viewDirection)),5.0)*_bianyuanliang*_node_1120.rgb)+((1.0 - node_4041)*_node_8894_var.rgb*_node_7850*i.vertexColor.rgb));
                float3 finalColor = emissive;
                fixed4 finalRGBA = fixed4(finalColor,(node_4041*i.vertexColor.a));
