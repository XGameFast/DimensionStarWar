...
////// Lighting:
                ...
                float2 node_1088 = ((i.uv0*float2(2,2))+node_4439.g*float2(-0.01,0.01));
                float4 node_7349 = tex2D(_node_8761,TRANSFORM_TEX(node_1088, _node_8761));
                float2 node_4510 = (i.uv0+node_4439.g*float2(0.005,-0.01));
                ...
                float3 finalColor = (((pow(1.0-max(0,dot(normalDirection, viewDirection)),1.5)*1.0)+tex2D( _GrabTexture, (0.05*(_zheshe - 0.5)*mul(tangentTransform, viewDirection).xy + lerp((sceneUVs * 2 - 1).rg,float2(node_443.r,node_443.g),_node_2028)).rg).rgb+_beiniuqutu_var.rgb)*_node_7650.rgb);
